...
export default function StudentView() {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`/student/${editStudent._id}`, form);
      alert("Updated!");
      setEditStudent(null);
  ...
  let rank = 1;
  rankedStudents.forEach((stu) => {
    stu.rank = stu.result === "Pass" ? rank++ : "-";
  });
      ...
      <tbody>
        {currentStudents.map((stu, index) => (
          ...
          >
            ...
            <td className="px-4 py-3 border">{stu.rank}</td>
          </tr>
        ))}
      </tbody>
    </table>
